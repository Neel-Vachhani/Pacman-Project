    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        likelyGhostDistances = {obj.argMax(): self.distancer.getDistance(obj.argMax(), pacmanPosition) for obj in livingGhostPositionDistributions} 
        closestGhostPos = min(likelyGhostDistances, key=likelyGhostDistances.get)
        bestMove = None
        bestDistance = float('inf')
        for move in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, move)
            newDistance = self.distancer.getDistance(closestGhostPos, successorPosition)
